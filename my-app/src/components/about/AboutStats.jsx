    ...
    <section className="py-20 sm:py-24 bg-[#0B0B0B] border-y border-[#242424] relative overflow-hidden">
      ...
      <Container className="relative z-10">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 lg:gap-12">
          {stats.map((stat, idx) => (
            <div key={idx} className="text-center space-y-1.5 sm:space-y-2 group">
              {/* 3D Depth Typography with Layered Shadows */}
              <div className="text-3xl sm:text-5xl lg:text-6xl font-extrabold text-white tracking-tight drop-shadow-[0_10px_20px_rgba(0,0,0,0.8)] [text-shadow:_0_0_25px_rgba(255,31,38,0.35)] transition-transform duration-300 group-hover:scale-105">
                <span className="text-[#FF1F26]">{stat.number.slice(0, -1)}</span>
                <span>{stat.number.slice(-1)}</span>
              </div>
              <div className="text-xs sm:text-base font-bold text-white tracking-wide">
                {stat.label}
              </div>
